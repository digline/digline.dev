...
**Ruled the same day: two modes, and the reader scoped to the English's diff. This supersedes the paragraph above's conclusion.** That paragraph stays because it is how the decision was reached. Letting the reader's notes reach outside the diff and sending those pages to "needs attention" would take auto-merge from roughly one run in two: 18 of the 40 changed pages were corrected. That does not protect anything. It puts a person back in the four-minute window nobody can stand in, it builds a queue nobody clears, and then the check gets switched off. So there are two modes with two contracts:
