...
**Decided, not incidental: a refused correction is lost whole.** Take a correction that fixes two calques inside the English's diff and rewrites a paragraph outside it. It loses all three, because the fallback keeps the text from before the correction. It could keep the two fixes and put back only the outside paragraph whenever the paragraphs line up, and it does not, on purpose. A pass that rewrote what nobody asked for has shown it does not keep to its instruction. Inside the diff the check cannot see whether it did the same thing there, so its changes inside the diff are unverified, not good fixes. Dropping them costs two calque fixes that the next run or a person can make. Keeping them would merge, unread and within four minutes, the output of a pass already caught not doing what it was told.
