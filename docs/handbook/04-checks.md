...
Only after those comes the question that needs a judge — "does this score agree with what the reader wanted?" — and in the newsletter project even that one turned out to be answerable without a model, because the reader's own marks are recorded. The check is fourteen lines of Python: *did the judge say ≥4 exactly when the reader marked it?* No second model, no noise from the check itself.
...
4. If a judged check fails on a case, look at the reason before you look at the prompt. Often it is the case that is wrong, not the system.
